  ...
  return (
    <>
      <Head>
        ...
        <meta name='description' content='This is an application to cheer you up using with ChatGPT API!' />
      ...
      <main className='my-5 p-5 max-w-screen-md m-auto text-gray-700'>
        <div className='text-right'>
          <button onClick={() => setEnglish(!english)}>
            {english ? (
              <Image src='/ja.png' alt='Japanese' width={25} height={25} className='' priority />
            ) : (
              <Image src='/en.png' alt='English' width={25} height={25} className='' priority />
            )}
          </button>
        </div>
        <form onSubmit={onSubmit}>
          ...
          <div className='flex justify-between'>
            <input
              ...
              name='message'
              className={
                'flex-auto bg-gray-50 border border-r-0 rounded-l-md focus:ring-sky-500 focus:border-sky-500 block w-full px-3 py-2' +
            ...
            <button
              type='submit'
              className={
                'flex-none w-24 text-white transition-colors focus:ring-2 focus:outline-none font-medium rounded-r-md text-sm text-center ' +
                (processing ? 'bg-sky-300 cursor-not-allowed' : 'bg-sky-500 hover:bg-sky-600 focus:ring-sky-300')
              }
              disabled={processing}
            >
              {result ? (
